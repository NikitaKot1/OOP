...
void WireframeModelDetails::transform(const Point &move_params, const Point &scale_params, const Point &rotate_params) {
    ...
    if (MainWindow().ui->rotate_all_box->isChecked())
    {
        this->setCenter(Point(0, 0, 0));
    }
    for (auto &point: points)
    {
        if (MainWindow().ui->rotate_all_box->isChecked())
             point.rotate(rotate_params.getX(), rotate_params.getY(), rotate_params.getZ(), Point(0, 0, 0));
        else
            point.rotate(rotate_params.getX(), rotate_params.getY(), rotate_params.getZ(), this->getCenter());
